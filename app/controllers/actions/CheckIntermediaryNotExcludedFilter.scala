...
class CheckIntermediaryNotExcludedFilterImpl @Inject()(
  ...
  override protected def filter[A](request: OptionalDataRequest[A]): Future[Option[Result]] = {
    ...
    val maybeEtmpExclusion: Boolean = checkExclusionsService
      .getLastExclusion(request.displayRegistration.exclusions.toList).isDefined
...
class CheckIntermediaryNotExcludedFilter @Inject()(
                                                 checkExclusionsService: CheckExclusionsService
                                               )(implicit ec: ExecutionContext) {

  def apply(): CheckIntermediaryNotExcludedFilterImpl =
    new CheckIntermediaryNotExcludedFilterImpl(checkExclusionsService)
}
