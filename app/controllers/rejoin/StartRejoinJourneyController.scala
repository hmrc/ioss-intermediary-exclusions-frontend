...
class StartRejoinJourneyController @Inject()(
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = cc.identifyAndGetOptionalDataWithExclusionInformation.async {
    implicit request =>
      ...
      registrationConnector.displayRegistration(request.intermediaryNumber).flatMap {
        case Right(registrationWrapper) =>
          ...
          if(canRejoin) {
            Redirect(frontendAppConfig.rejoinSchemeUrl).toFuture
          } else {
